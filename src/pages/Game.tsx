...
const Game = () => {
      ...
      timerRef.current = setInterval(() => {
        ...
        if (timeRef.current === 0) {
          ...
          if (scoreRef.current >= 100) {
            toast({ title: "🎉 You Win!", description: `You tapped ${scoreRef.current} times!` });
          } else {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-6 p-6">
      ...
      <h2 className="text-3xl text-primary">Tap 100 Times in 20 Seconds</h2>
      ...
      {!finished ? (
        <Button
          onClick={handleTap}
          className={`h-32 w-32 rounded-full text-2xl shadow-[var(--shadow-button)] transition-transform ${tapping ? "animate-tap-pulse" : ""}`}
